...
class TruroDatabaseHelper {
  ...
  void _createTable(Database db, int newVersion) async {
    var query =
        'CREATE TABLE $musicTable (id STRING PRIMARY KEY, service_date INT, service_time INT, rehearsalTime INT, serviceType String, musicType STRING, title STRING, composer STRING, link STRING, conductor STRING, serviceOrganist STRING, colour STRING)';
  ...
  void _upgradeTable(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 5) {
      db.execute("ALTER TABLE $musicTable ADD COLUMN conductor string;");
    }
  ...
  Future<int> insertMusic(Music music) async {
    ...
    int result;
    try {
      result = await db.insert(musicTable, music.toIdMap());
    } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> getAllServices() async {
    ...
    result = await db.rawQuery('SELECT *, service_date FROM $musicTable');
